...
RUN apk add --no-cache autoconf automake c-ares-dev curl gcc libc-dev libevent-dev libtool make openssl-dev pkgconfig
...
RUN cd /pgbouncer && ./configure --prefix=/usr --with-cares && make -j$(nproc) pgbouncer && strip pgbouncer

FROM alpine:3.22

RUN apk add --no-cache c-ares libevent postgresql-client && \
